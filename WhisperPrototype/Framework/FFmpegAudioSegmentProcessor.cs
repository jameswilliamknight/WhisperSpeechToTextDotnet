...
namespace WhisperPrototype.Framework
{
    public class FFmpegAudioSegmentProcessor : IAudioSegmentProcessor
    {
        ...
        public FFmpegAudioSegmentProcessor(AppSettings settings)
        {
            _settings = settings;
        }

        public async Task<Stream> GetSegmentStreamAsync(string parentWavFilePath, AudioSegment segment, int segmentIndex, int totalSegments, string workingDirectory)
        {
            var segmentsDirectory = Path.Combine(workingDirectory, "segments");
            ...
            var segmentOutputWavPath = Path.Combine(segmentsDirectory, segmentFileName);

            // Verbose: Show segment extraction details
            if (_settings.Verbosity >= VerbosityLevel.Verbose)
            {
                AnsiConsole.MarkupLine($"[grey]   Extracting segment {segmentIndex + 1}/{totalSegments} ({segment.StartTime:g} to {segment.EndTime:g})[/]");
            }
            
            // Debug: Show full details including filenames
            if (_settings.Verbosity >= VerbosityLevel.Debug)
            {
                AnsiConsole.MarkupLine($"[grey]   Segment file: {Markup.Escape(segmentFileName)}[/]");
            }
            ...
            if (segment.Duration <= TimeSpan.Zero)
            {
                ...
                {
                    AnsiConsole.MarkupLine($"[yellow]   Warning: Segment {segmentIndex + 1} has zero or negative duration. Skipping.[/]");
                }
                return Stream.Null;
            }
            ...
            if (File.Exists(segmentOutputWavPath))
            {
                if (_settings.Verbosity >= VerbosityLevel.Debug)
                {
                    AnsiConsole.MarkupLine($"[grey]   Deleting existing segment file[/]");
                }
            ...
            // Debug: Show full FFmpeg command
            if (_settings.Verbosity >= VerbosityLevel.Debug)
            {
            ...
            {
                // Always show errors
                AnsiConsole.MarkupLine($"[red]   Error: FFmpeg failed to extract segment {segmentIndex + 1}. Exit Code: {process.ExitCode}[/]");
                if (_settings.Verbosity >= VerbosityLevel.Verbose)
                {
                ...
                if (_settings.Verbosity >= VerbosityLevel.Debug)
                {
                    AnsiConsole.MarkupLine($"[red]   Output Path: {Markup.Escape(segmentOutputWavPath)}");
                    AnsiConsole.MarkupLine($"[red]   FFmpeg Stderr:\n{Markup.Escape(ffmpegErrorOutput)}[/]");
                }
                return Stream.Null;
            }

            // Debug: Confirm successful extraction
            if (_settings.Verbosity >= VerbosityLevel.Debug)
            {
                AnsiConsole.MarkupLine($"[green]   Extracted segment {segmentIndex + 1}[/]");
            }
    ...
    {
        public Task<Stream> GetSegmentStreamAsync(string parentWavFilePath, AudioSegment segment, int segmentIndex, int totalSegments, string workingDirectory)
        {

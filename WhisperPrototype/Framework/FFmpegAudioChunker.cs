...
namespace WhisperPrototype.Framework
{
    public class FFmpegAudioChunker : IAudioChunker
    {
        ...
        public async Task<List<AudioSegment>> DetectSpeechSegmentsAsync(string wavFilePath, VADParameters parameters)
        {
            ...
            {
                AnsiConsole.MarkupLine($"[yellow]    Step 2: Splitting speech segments...[/]");
            }
            ...
            for (var i = 0; i < silencePoints.Count; i += 2)
            {
                ...
                if (silenceStart > currentPosition)
                {
                    ...
                    if ((paddedSpeechEnd - paddedSpeechStart) >= parameters.MinSpeechSegmentSeconds)
                    {
                        ...
                        if (_settings.Verbosity >= VerbosityLevel.Normal)
                        {
                            var segment = speechSegments.Last();
                            var start = $"{segment.StartTime.Hours:D1}:{segment.StartTime.Minutes:D2}:{segment.StartTime.Seconds:D2}.{segment.StartTime.Milliseconds:D3}";
                            var end = $"{segment.EndTime.Hours:D1}:{segment.EndTime.Minutes:D2}:{segment.EndTime.Seconds:D2}.{segment.EndTime.Milliseconds:D3}";
                            var duration = $"{segment.Duration.Hours:D1}:{segment.Duration.Minutes:D2}:{segment.Duration.Seconds:D2}.{segment.Duration.Milliseconds:D3}";
                            AnsiConsole.MarkupLine($"[grey]        {start}  ->  {end}  =  {duration}[/]");
                        }
                    }
                    else
                    {
